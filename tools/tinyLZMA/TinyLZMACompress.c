...
// bitreverse builtin, maybe someday. Please, someday.
// static inline uint32_t bitsReverse(uint32_t bits, const uint32_t bit_count) {
...
static RangeEncoder_t newRangeEncoder (uint8_t *dest, const size_t destLength) {
    ...
    coder.cache_size  = 1;
    coder.p_dst       = dest;
    coder.p_dst_limit = dest + destLength;
    coder.overflow    = 0;
    return coder;
}
...
static void rangeEncodeBit (RangeEncoder_t *e, uint16_t *p_prob, const uint8_t bit) {
    const uint32_t prob = *p_prob;

    rangeEncodeNormalize(e);

    const uint32_t bound = (e->range >> RANGE_CODE_N_BIT_MODEL_TOTAL_BITS) * prob;
...
static void rangeEncodeByteMatched (RangeEncoder_t *e, uint16_t *p_prob, uint32_t byte, uint32_t match_byte) {
    uint32_t treepos = 1, off0 = 0x100;
    for (uint32_t i = 0; i<8; i++) {
        const uint8_t bit = (uint8_t)(1 & (byte >> 7));
        byte <<= 1;
        match_byte <<= 1;
        const uint32_t off1 = off0;
        off0 &= match_byte;
...
static void updateHashTable (const uint8_t *p_src, const size_t src_len, const size_t pos, size_t hash_table [][HASH_LEVEL]) {
    ...
    for (uint32_t i = 0; i<HASH_LEVEL; i++) {
...
static uint32_t lenDistScore (const uint32_t len, const uint32_t dist, const uint32_t rep0, const uint32_t rep1, const uint32_t rep2, const uint32_t rep3) {
    #define D 12
...
static void lzSearchMatch (const uint8_t *p_src, const size_t src_len, const size_t pos, size_t hash_table [][HASH_LEVEL], uint32_t *p_len, uint32_t *p_dist) {
    ...
    *p_dist = 0;
    ...
    uint32_t score1 = lenDistScore(0, 0xFFFFFFFF, 0, 0, 0, 0);

    for (uint32_t i = 0; i<HASH_LEVEL+2; i++) {
        const size_t ppos = (i<HASH_LEVEL) ? hash_table[hash][i] : (pos-1-(i-HASH_LEVEL));
        if (ppos != INVALID_HASH_ITEM && ppos < pos && (pos - ppos) < LZ_DIST_MAX_PLUS1) {
            for (j=0; j<len_max; j++)
                if (p_src[pos+j] != p_src[ppos+j])
                    break;
            const uint32_t score2 = lenDistScore(j, (pos - ppos), 0, 0, 0, 0);
...
static void lzSearchRep (const uint8_t *p_src, const size_t src_len, const size_t pos, const uint32_t rep0, const uint32_t rep1, const uint32_t rep2, const uint32_t rep3, const uint32_t len_limit, uint32_t *p_len, uint32_t *p_dist) {
...
static void lzSearch (const uint8_t *p_src, const size_t src_len, const size_t pos, const uint32_t rep0, const uint32_t rep1, const uint32_t rep2, const uint32_t rep3, size_t hash_table [][HASH_LEVEL], uint32_t *p_len, uint32_t *p_dist) {
    uint32_t rlen, rdist;
    uint32_t mlen, mdist;
...
static uint8_t isShortRep (const uint8_t *p_src, size_t src_len, const size_t pos, const uint32_t rep0) {
    return (pos >= rep0 && (p_src[pos] == p_src[pos-rep0])) ? 1 : 0;
}

// Lempel-Ziv Markov chain Algorithm encoder
...
// Lempel-Ziv Markov chain Algorithm compression
